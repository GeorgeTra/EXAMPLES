...
Если k1 = k2, то прямые параллельны!! Если k различны, то прямые пересекаются.
Если b1 = b2, то прямые пересекаются в точке (0;b).
Алгоритм решения такой: В одно из уравнений вставляем вместо "y" второе уравнение 
и находим "х". Потом с помощью любого уравнения находим "y".*/

int EnterInt(string Text)
// Эта функция печатает в консоли нужный текст и возвращает введенное пользователем число
{
    Console.Write(Text);
...
Console.Clear();
Console.WriteLine("Введите значение переменных для уравнений: ");
